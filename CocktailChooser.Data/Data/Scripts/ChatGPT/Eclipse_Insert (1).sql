...
SELECT 'Eclipse', 'Fruity and Refreshing', 'Muddle raspberries in the base of a shaker. Add other ingredients to the shaker with ice. Shake. Strain into an ice-filled Collins glass. Garnish with a raspberry and sprig of mint.', (SELECT Id FROM GlassTypes WHERE Name = 'Collins'), (SELECT Id FROM CocktailTimePeriods WHERE Name = 'Modern'), 0, 1
...
SELECT 'Raspberry', 1, NULL, NULL
WHERE NOT EXISTS (SELECT 1 FROM Ingredients WHERE Name = 'Raspberry');
...
SELECT CocktailId, (SELECT Id FROM Ingredients WHERE Name = 'Raspberry'), (SELECT Id FROM Amounts WHERE MeasurementName = '2')
FROM LastCocktail
WHERE NOT EXISTS (SELECT 1 FROM CocktailIngredients WHERE CocktailId = (SELECT Id FROM Cocktails WHERE Name = 'Eclipse') AND IngredientId = (SELECT Id FROM Ingredients WHERE Name = 'Raspberry') AND AmountId = (SELECT Id FROM Amounts WHERE MeasurementName = '2'));
